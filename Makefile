...
ASM_SRCS = kernel_interface.s
C_SRCS = main.c

# Simple Monitor sources
C_SRCS += prompt.c monitor.c uart.c parser.c dump.c syscall.c terminal.c \
	decoder/decoder.c decoder/STM32F30x_decoder.c \
	printf_stdarg.c mytest.c
...
C_SRCS += $(CMSIS_DEV_SRC)/system_stm32f3xx.c

# Location of HAL drivers
HAL     = Drivers/STM32F3xx_HAL_Driver
HAL_INC = $(HAL)/Inc
HAL_SRC = $(HAL)/Src
C_SRCS   += $(HAL_SRC)/stm32f3xx_hal_rcc.c \
...
C_SRCS   += $(USB_CORE_SRC)/usbd_core.c \
...
C_SRCS   += $(USB_CLASS_SRC)/usbd_cdc.c \
...
BSP_INC = $(BSP)
BSP_SRC = $(BSP)
C_SRCS   += $(BSP)/stm32f3_discovery.c \
...
CMP_INC = $(CMP)
CMP_SRC = $(CMP)
C_SRCS   += $(CMP)/lsm303dlhc/lsm303dlhc.c \
	  $(CMP)/l3gd20/l3gd20.c
...
DEBUG_ASSEMBLY = $(addsuffix .debug.s,$(basename $(C_SRCS)))  # Don't just call them '.s' files to simplify the 'make clean' rule.
DEPS = $(addprefix deps/,$(C_SRCS:.c=.d))
...
%.debug.s : %.c deps
	$(CC) $(CFLAGS) -S -o $@ $< -MMD -MF deps/$(*F).d

$(PROJ_NAME).elf: $(ASM_SRCS) $(OBJS) $(STARTUP)
